// my-medusa-store/src/modules/marketplace/migrations/Migration20251205012742.ts
...
export class Migration20251205012742 extends Migration {
  ...
  override async up(): Promise<void> {
    // Drop constraints cũ nếu có (giữ nguyên logic cũ của bạn)
    ...
    // 1. Tạo bảng CARRIER (Thêm cột "metadata")
    this.addSql(`create table if not exists "carrier" (
        "id" text not null, 
        "name" text not null, 
        "code" text not null, 
        "api_url" text null, 
        "metadata" jsonb null,  -- <--- ĐÃ THÊM DÒNG NÀY
    ...
    // 2. Tạo bảng SELLER (Thêm cột "metadata")
