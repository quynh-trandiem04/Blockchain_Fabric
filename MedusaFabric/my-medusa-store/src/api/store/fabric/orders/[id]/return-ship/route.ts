// src/api/store/fabric/orders/[id]/return-ship/route.ts
...
export const POST = async (req: MedusaRequest, res: MedusaResponse) => {
  ...
  try {
    ...
    if (!ALLOWED_ROLES.includes(role)) return res.status(403).json({ message: "Forbidden" });
    if (!companyCode) return res.status(400).json({ message: "Missing Company Code" });

    // 3. Call Fabric
    console.log(`[API] ShipReturn for ${id} by ${companyCode}`);
    await fabricService.shipReturn(id, companyCode);

    return res.json({ success: true, message: "Đã xác nhận lấy hàng hoàn thành công!" });
