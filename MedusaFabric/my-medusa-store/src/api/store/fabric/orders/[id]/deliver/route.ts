// my-medusa-store\src\api\store\fabric\orders\[id]\deliver\route.ts
...
export const POST = async (req: MedusaRequest, res: MedusaResponse) => {
  ...
  try {
    ...
    if (!companyCode) {
        return res.status(400).json({ message: "Lỗi: Tài khoản Shipper thiếu mã công ty (company_code)." });
    }

    // 3. Call Fabric (Confirm Delivery - Prepaid)
    console.log(`[API] ConfirmDelivery for ${id} | By: ${user.email} | Org: ${companyCode}`);
    
    // 🔥 QUAN TRỌNG: Truyền companyCode xuống để Chaincode verify
    await fabricService.confirmDelivery(id, companyCode);

    res.json({ 
        success: true,
        message: "Giao hàng thành công! (Prepaid)" 
    });
