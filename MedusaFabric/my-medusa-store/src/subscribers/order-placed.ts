...
const TAX_API_BASE = process.env.TAX_API_BASE || "http://192.168.245.11:3333";

function sleep(ms: number) {
    return new Promise((r) => setTimeout(r, ms));
}

async function tryGetTax(orderKey: string) {
    const url = `${TAX_API_BASE}/tax/order/${encodeURIComponent(orderKey)}`;
    const res = await fetch(url);
    if (!res.ok) return null;
    return await res.json();
}

// Poll vì tax service đọc block event nên có độ trễ vài trăm ms – vài giây
async function pollTaxForOrder(splitOrderID: string, attempts = 12, intervalMs = 800) {
    // 2 key candidate: raw splitOrderID hoặc ORDER_ + splitOrderID
    const candidates = [splitOrderID, `ORDER_${splitOrderID}`];

    for (let i = 0; i < attempts; i++) {
        for (const k of candidates) {
            const rec = await tryGetTax(k);
            if (rec?.tax?.totalTax) return rec;
        }
...
export default async function orderPlacedHandler({
    ...
    try {
        ...
        // =================================================================
        // Prepare baseMetadata and taxMap outside the loop to avoid overwriting
        const baseMetadata = order.metadata || {};
        const taxMap: Record<string, any> = baseMetadata.tax_map || {};
        ...
        for (const [sellerID, items] of Object.entries(sellerGroups)) {
            ...
            try {
                ...
                const txId = await fabricService.createOrder(
                    payload,
                    sellerID,
                    payload.amount_total 
                ); console.log(`[${splitOrderID}] Ghi thành công! TX: ${txId}`);
                // ===================
                // BƯỚC 6: GỌI TAX API + LƯU VÀO MEDUSA METADATA
                // ===================
                try {
                    const taxRec = await pollTaxForOrder(splitOrderID);

                    if (taxRec) {
                        console.log(`[${splitOrderID}] Tax ready:`, taxRec.tax, `block=${taxRec.block}`);

                        // Update taxMap for this splitOrderID
                        taxMap[splitOrderID] = {
                            tax: taxRec.tax,
                            block: taxRec.block,
                            txId: taxRec.txId,
                        ...
                        // Update metadata using Order Module (recommended)
                        ...
                        console.warn(`[${splitOrderID}] Tax not ready after polling`);
                    }
                } catch (e: any) {
                    console.warn(`[${splitOrderID}] Tax fetch/store failed: ${e.message}`);
                }
